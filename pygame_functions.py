...
class newLabel(pygame.sprite.Sprite):
    def __init__(self, text, fontSize, font, fontColour, xpos, ypos, background, centre):
        pygame.sprite.Sprite.__init__(self)
        self.text = text
...
def makeLabel(text, fontSize, xpos, ypos, fontColour='black', font='Arial', background="clear", centre=False):
    # make a text sprite
    thisText = newLabel(text, fontSize, font, fontColour, xpos, ypos, background, centre)
    return thisText


def moveLabel(sprite, x, y, centre=False):
    if centre:
